...
export default function FormProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const savedFormId = localStorage.getItem('egyptFormId');
      const savedEmailAddress = localStorage.getItem('egyptEmailAddress');
      const savedFormData = localStorage.getItem('egyptFormData');
      ...
      localStorage.removeItem('egyptFormData');
      localStorage.removeItem('egyptStepCompletion');
    }
  }, []);
  ...
  useEffect(() => {
    try {
      if (formId) {
        localStorage.setItem('egyptFormId', formId);
        console.log('FormProvider - Saved formId to localStorage:', formId);
      }
      if (emailAddress)
        localStorage.setItem('egyptEmailAddress', emailAddress);
      localStorage.setItem('egyptFormData', JSON.stringify(formData));
      localStorage.setItem(
        'egyptStepCompletion',
        JSON.stringify(isCompleted)
      );
      localStorage.setItem('egyptCurrentStep', currentStep);
  ...
  const setFormIdWithValidation = (id: string) => {
    ...
    setFormId(id);
    localStorage.setItem('egyptFormId', id);
  };
